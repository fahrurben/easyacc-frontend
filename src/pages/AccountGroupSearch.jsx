...
class AccountGroupSearch extends Component {
  ...
  async update(id) {
    console.log(id);
    this.props.history.push('/accountgroup/update/'+id);
  }
  ...
  async goToPageService(page) {
    ...
    try {
      let response = await fetchHelper(`${apiUrl}/api/accountgroups/find?sortBy=id&sortOrder=ASC&page=${page}&size=10`,{
        method: "POST",
        body: JSON.stringify({})
      });
    ...
    return (
      <div className="columns">
        <div className="column">
          <h1 className="title">Account Group <Link to="/accountgroup/create" class="button is-medium is-primary">Create</Link></h1>
                ...
                data && data.map((row, i) => {
                  return (
                    <tr>
                      <td>{row.name}</td>
                      <td>{row.code}</td>
                      <td><button className="button is-small" onClick={() => this.update(row.id)}>Update</button></td>
                    </tr>
                  )
                })
              }
