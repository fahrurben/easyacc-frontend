...
class MoveCreate extends Component {
  ...
  saveLine = (ev) => {
    ...
    if (lineIdx !== null) {
      data[lineIdx] = { id: data[lineIdx].id, ...line };
    } else {
      data.push(line);
    }
    this.setState({
  ...
  async componentDidMount() {
    ...
    try {
      let response = await fetchHelper(`${apiUrl}/api/moves/${this.id}`,{
        method: "GET"
      });

      response = await response.json();

      let lines = response.lines.map((val) => {
        return {
          id: val.id,
      ...
      let datePosted = moment(response.datePosted).format("YYYY-MM-DD");
  ...
  async submit(ev) {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      body.lines = lines;
      try {
        await fetchHelper(`${apiUrl}/api/moves/${this.id}`,{
          method: "PUT",
